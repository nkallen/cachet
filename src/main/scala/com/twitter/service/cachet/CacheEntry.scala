...
object CacheEntry extends Function[ResponseBuffer, CacheEntry] {
  def apply(responseWrapper: ResponseBuffer) = new CacheEntry(responseWrapper)
}

class CacheEntry(val responseWrapper: ResponseBuffer) {
